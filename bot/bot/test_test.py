    ...
    try :   
            ...
            for filename in os.listdir(path) :
                ...
                with open(os.path.join(path,filename),'r') as f :
                     ...
                     embeddings_ques = EmbeddingFunction(texts=questions)
                     embeddings_doc = EmbeddingFunction(texts=documents)
                     yield {'file_vector':embeddings_doc,'file_name':filename,'file_content':documents,'question_vector':[{"vector":embeddings_ques }]}
...
def new_main():
    ...
    with tqdm.tqdm(total=35,desc="Bulk indexing progress") as progress :
         ...
         for success , info in streaming_bulk(client=ES,index='question-index-3',actions=genActions(APIKEY=APIKEY,path=path,model_name=model_name,Function=Function)) :
...
# }
    ...
    response = ES.search(index='question-index-3', body=query)
    for hit in response['hits']['hits'] :
        print(f'file name {hit["_source"]["file_name"]} match score : {hit["_score"]} \n') 
